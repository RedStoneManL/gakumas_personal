"""Check v4's packaged CLI against an existing real full-produce checkpoint.
...
archive = artifacts / 'gakumas-colab-training-v4.zip'
root, manifest, manifest_hash = extract_verified_bundle(archive, artifacts)
output = Path(tempfile.mkdtemp(prefix='v4-migration-check-', dir=artifacts))
...
(workspace / 'rl/training/colab/validation/v4-packaged-migration.json').write_text(
